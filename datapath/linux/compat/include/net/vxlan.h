...
enum {
	VXLAN_PARSE_OK,
	VXLAN_PARSE_ERR,
	VXLAN_PARSE_CONSUMED
};

struct vxlan_sock;
struct vxlanhdr;
typedef void (vxlan_rcv_t)(struct vxlan_sock *vs, struct sk_buff *skb, __be32 key);
typedef void (vxlan_ctor_t)(struct sk_buff *skb, __be32 vni);
typedef struct vxlanhdr *(vxlan_parse_t)(struct sock *sk, struct sk_buff *skb, int *retval);

/* per UDP socket information */
struct vxlan_sock {
	struct hlist_node hlist;
	vxlan_rcv_t	 *rcv;
        vxlan_ctor_t     *construct;
        vxlan_parse_t    *parse;
	void		 *data;
