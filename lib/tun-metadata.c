...
tun_meta_init(void)
{
    ...
    table->next_ofs = 0;
    table->count = 0;
    unixctl_command_register("tnl/meta/show", "", 0, 0, tun_meta_print_keys, NULL);
    unixctl_command_register("tnl/meta/flush", "", 0, 0, tun_meta_remove_all, NULL);
    initialized = true;
}
...
/* xlate actions use this api to get an offset into flow.tun_metadata */
int action_find_or_add_tun_meta_entry(const uint8_t metadata[TUN_METADATA_LEN],
                                      int len)
{
    struct tun_meta_entry *entry = find_or_add_tun_meta_entry(metadata, len);
    if (entry) {
...
{
    uint16_t len, mofs = 0, nofs = 0;
    struct geneve_opt *opt;

    while (tun_metadata_get_len(mofs, &len)) {
        if (tun_metadata_valid(metadata + mofs, len, mofs)) {
            if ((len - 3) % 4) {
                mofs += len;
                ovs_assert(mofs < TUN_METADATA_LEN);
                continue;
            }
            opt = (struct geneve_opt *)(nlattr + nofs);
            memcpy(nlattr + nofs, metadata + mofs, 3);
            opt->length = (len - 3) / 4;
            opt->r1 = 0;
            opt->r2 = 0;
            opt->r3 = 0;
            memcpy(opt->opt_data, metadata + mofs + 3, len - 3);
            nofs += (len + 1);
        }
        mofs += len;
        ovs_assert(mofs < TUN_METADATA_LEN);
